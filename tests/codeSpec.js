describe('Objects', function() {
  /*Model a Credit Card in english.
  “How would you give your credit card info to someone?”.
  This naturally creates a “key value pair”
Key/Value Pairs
Object Literal
...
*/
  ...
  describe('Use key value pairs', function() {
    ...
    it('can have values that are arrays, like [1,2,3]', function() {
      expect(makeArrayObject()).toEqual({
        arrayKey: [1, 2, 3]
      });
    });

    it('can have values that are functions, like sayHi', function() {
      expect(makeFunctionObject().sayHi()).toEqual("Hi");
    });

    it('can nest values deeply', function() {
      expect(makeObjception()).toEqual({
        objectKey: {
          inceptionLevel1: {
            inceptionLevel2: {
    ...
    it('uses commas to separate multiple properties', function() {
      expect(makeMultikeyObject()).toEqual({prop1: "So beaucoup", prop2: "So mucho", prop3: "So many"});
    });
  });
  ...
  it('Can be created and added to in any order', function() {
    expect({first: 'first', second: 'second', third: 'third'}).toEqual({second: 'second', third: 'third', first: 'first'});
  });

  it('Can have properties deleted', function() {
    expect(deleteSecond({first: 'first', second: 'second', third: 'third'})).toEqual({first: 'first', third: 'third'});
  });
